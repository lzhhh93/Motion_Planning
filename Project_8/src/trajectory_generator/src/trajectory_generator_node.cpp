...
void changeState(STATE new_state, string pos_call) {
  string state_str[5] = {"INIT", "WAIT_TARGET", "GEN_NEW_TRAJ", "EXEC_TRAJ",
                         "REPLAN_TRAJ"};
  int pre_s = int(exec_state);//take the int of Enum
...
bool trajGeneration() 
{
   ...
   * **/
   int Heudirector = 2; 
   // Heudirector: 1-Manhatten Dist,2-Euclidean Dist,3-Diagonal Dist
  _astar_path_finder->AstarGraphSearch(start_pt, target_pt, Heudirector);
  ...
  if( grid_path[0] != grid_path[ grid_path.size()-1 ]){
  
    grid_path = _astar_path_finder->pathSimplify(grid_path, _path_resolution); //_path_resolution == 0.2  
  
  }
  else{
    ROS_WARN("Destination is arrived");
  }
...
void trajOptimization(Eigen::MatrixXd path) {
   ...
   *
   * **/
  unsafe_segment = _astar_path_finder->safeCheck(_polyCoeff, _polyTime, _map_upper,_map_lower);
  vector<int> unsafe_part;

  MatrixXd repath = path;
  int count = 0;
  while (unsafe_segment[0] != -1) {
    /**
     *
     * STEP 3.4:  reoptimize
    ...
    for(int i = 1; i< (int)unsafe_segment.size();i++)
    {
      if( unsafe_segment[i] == 1)
      {
        count += 1;
      }
    }

    MatrixXd path_(int(repath.size()+count),3);
    

  }
  // visulize path and trajectory
cout<<"Coord of Path"<<endl;
visPath(repath);

for (int i = 0; i < unsafe_segment.size(); i++)
{
  if (unsafe_segment[i] == 1)
  {
    unsafe_part.push_back(i);
    ROS_WARN("%f th part of the traj is unsafe", (double)i);
  }
}
...
VectorXd timeAllocation(MatrixXd Path) {
  ...
  for (int i = 0; i < (int)(Path.rows() - 1); i++)
  {
    d_x = Path(i + 1, 0) - Path(i, 0);
    d_y = Path(i + 1, 1) - Path(i, 1);
    d_z = Path(i + 1, 2) - Path(i, 2);
    dis = sqrt(pow(d_x, 2) + pow(d_y, 2) + pow(d_z, 2));
    time(i) = 1 + dis/2;
    //vmax = 2, a = 2 or -2;
  }
...
int main(int argc, char **argv) {
  ...
  // 10 times will be called before the execCallback is called again 
  // as long as "odom", his advertiser, keeps sending messages, which is always true.
  
  // start_pt of the drone is within odom: 
  // In rcvOdomCallback, odom's pos & vel 
  // would be given to the global variables:
  // Vector3d odom_pt, odom_vel, start_pt, target_pt, start_vel;(line 48)
  
  _map_sub = nh.subscribe("local_pointcloud", 1, rcvPointCloudCallBack);

  // 1 time will be called after the rcvOdomCallback has been called 5 times.
  // as long as "local_pointcloud", his advertiser, keeps sending messages, which is also alwasys true.

  _pts_sub = nh.subscribe("waypoints", 1, rcvWaypointsCallBack);
  // 1 time will be called before the execCallback is called again, if "waypoints" sends message.
  // "waypoints" will only be active, when target is sended from rviz.
  ...
  bool status = ros::ok();
  while (status) {
    ros::spinOnce(); //after one iteration,break
    status = ros::ok();
    rate.sleep(); 
  }
