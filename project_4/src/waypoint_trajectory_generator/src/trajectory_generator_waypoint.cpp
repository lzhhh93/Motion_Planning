...
*/
//VectorXd TrajectoryGeneratorWaypoint::reverseArray(VectorXd a)
//{
//    return {a.rbegin(), a.rend()};
//}

Eigen::MatrixXd TrajectoryGeneratorWaypoint::PolyQPGeneration(
            const int d_order,                    // the order of derivative: equivalent to Minimum_index in my code 
            const Eigen::MatrixXd &Path,          // waypoints coordinates (3d)
            const Eigen::MatrixXd &Vel,           // boundary velocity
            const Eigen::MatrixXd &Acc,           // boundary acceleration
            const Eigen::VectorXd &Time)          // time allocation in each segment
{
    ...
    int p_num1d   = p_order + 1;                  // the number of variables in each segment:8
