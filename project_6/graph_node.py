...
class Node:
    ...
    def connect_to_graph(self, grid): 
        # be referred in build up graph
        for u in ACTION_SPACE:
            self.next_prob_9.append(self.control(u[0], u[1], grid, success=True))
            # control():return the key of the corresponding graph point that is driven by u
    ...
    # limit vel under 4 without changing the sign of it
    
    def safety_constraints(self, px2, py2, grid):
        assert  0 <= self.px < grid.shape[0]
        assert  0 <= self.py < grid.shape[1]
        # pass if the pos of self within the race track map
        ...
        # cuz it takes the ceil of waypoints, may hit the obs when y increases less than x
        # astype(np.int): take the int part

        for idx in range(way_points.shape[0]):
            # shape: dimension
            point = way_points[idx]
            if (0 <= point[0] < grid.shape[0]) and (0 <= point[1] < grid.shape[1]):
                # if waypoint is within the shape of x and y axis
                if grid[point[0], point[1]] == FINISH:
                    # if the point is within the goal
                    return FINISH, point
    ...
    def control(self, ux, uy, grid, success):
        ...
        if status == FREE:
            ...
            #if outbound then start from a random start point
