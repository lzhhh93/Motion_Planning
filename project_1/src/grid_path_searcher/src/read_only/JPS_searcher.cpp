...
inline void JPSPathFinder::JPSGetSucc(GridNodePtr currentPtr, vector<GridNodePtr> & neighborPtrSets, vector<double> & edgeCostSets)
{
    ...
    for( int dev = 0; dev < num_neib + num_fneib; ++dev) {
        ...
        if( dev < num_neib )
        // for the neighbor that are always added; when dev < num_neib : check whether the neighbor to be added is obstacle
         {
        ...
        else// for the neighbor tha can be forced neighbor
        {
...
void JPSPathFinder::JPSGraphSearch(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt,int _Heudirector)
{
        ...
        */         
        for(int i = 0; i < (int)neighborPtrSets.size(); i++){
            ...
            */
            ...
            if (neighborPtr->id != 1)
            {   //discover a new node which is not in open set
                /*
                *
                *
